...
public class EnglishUnitsFilter implements IUnitsFilter, Runnable {
    ...
    private static final Logger LOG = Logger.getLogger(EnglishUnitsFilter.class);

    /**
     * Constructor takes reference to main dictionary.
     *
     * @param aDict reference to dictionary.
     */
    ...
    private class DelJednWnSynRel implements Runnable {
        ...
        public void run() {
            StopWatch watch = new StopWatch(this.getClass(),
                    "Removing invalid relations between Synset and WordNet units...");
            watch.start();
            ...
            if ((allWnUnitSynRels - wnUnitSynsetRels.size()) < 1) {
                ...
                LOG.info("Number of removed invalid relations: " + (allWnUnitSynRels - wnUnitSynsetRels.size()));
            }
    ...
    private class DelLexRel implements Runnable {
        ...
        @Override
        public void run() {
            StopWatch watch = new StopWatch(this.getClass(), "Removing english lexical relations");
            watch.start();
            ...
            for (LexicalRelRaw l : lexicalRelations) {
                synchronized (engUnitsIDs) {
                    if (engUnitsIDs.contains(l.getChild()) || engUnitsIDs.contains(l.getParent())) {
                        englishLexRel.add(l);
                    }
                }
            }
            ...
            if ((allLexRelCount - lexicalRelations.size()) < 1) {
                ...
                LOG.info("Number of english lexical relations removed : " + (allLexRelCount - lexicalRelations.size()));
                watch.stop();
            }
        }
    }
